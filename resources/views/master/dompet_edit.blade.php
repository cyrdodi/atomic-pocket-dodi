<x-layout>
  <x-header title="Dompet" subtitle="Ubah">
  ...
  <x-card>
    <div class="w-1/2">
      <form action="" method="post">
        @csrf
        <input type="text" name="id" value="{{ $dompet->id }}" hidden>
        <div class="mb-6">
          ...
          <input type="text"
            ...
            id="nama" value="{{ $dompet->nama }}" required minlength="5">
          @error('nama')
        ...
        <div class="mb-6">
          ...
          <input type="text"
            ...
            name="referensi" id="referensi" value="{{ $dompet->referensi }}">
          @error('referensi')
          <p class="mt-2 text-xs text-red-500">{{ $message }}</p>
          @enderror
        </div>
        ...
        <div class="mb-6">
          ...
          <input type="text"
            ...
            name="deskripsi" id="deskripsi" value="{{ $dompet->deskripsi }}" maxlength="100">
          @error('deskripsi')
          <p class="mt-2 text-xs text-red-500">{{ $message }}</p>
          @enderror
        </div>
        ...
        <div class="mb-6">
          ...
          <select name="status" id="status" class="w-full p-2 border border-gray-400 rounded-lg">
            ...
            <option value="{{ $item->id }}" {{ $item->id === $dompet->status_id ? 'selected' : "" }}>{{ $item->nama }}
            </option>
            @endforeach
          </select>
          </select>
